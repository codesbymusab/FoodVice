import { AIRecommendationsDTO } from "../../dtos/input/AI/RecommendationsDTO";
import { AISummaryDTO } from "../../dtos/input/AI/SummaryDTO";
import { AIChatDTO } from "../../dtos/input/AI/ChatDTO";

function formatRestaurantsForPrompt(restaurants: Array<Record<string, any>>) {
...
function buildRecommendationsPrompt(payload: any) {


    const location = payload.location || {};
    const locationText = `lat:${location.lat || "unknown"}, lon:${location.lon || "unknown"}`;
    const restaurants = formatRestaurantsForPrompt(Array.isArray((payload as any).restaurants) ? (payload as any).restaurants : []);
...
function buildSummaryPrompt(payload: any) {

    const reviews = Array.isArray((payload as any).reviews) ? (payload as any).reviews : [];
    const reviewSummaryInput = reviews.map((review: Record<string, any>) => ({
        text: review.text || "",
...
function buildChatPrompt(payload: any) {


    const location = payload.location || {};
    const restaurants = formatRestaurantsForPrompt(Array.isArray((payload as any).restaurants) ? (payload as any).restaurants : []);
    const conversation = (payload as any).messages
        .map((message: Record<string, any>) => `${message.role}: ${message.content}`)
        .join("\n");
...
export function buildPrompt(type: string, payload: any) {
    switch (type) {
